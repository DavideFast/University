  ...
  if (pesato) {
    ...
    for (let i = 0; i < grafo.links.length; i++) {
      graph.addEdgeWithKey(
        grafo.links[i].src + " - " + grafo.links[i].dst,
        grafo.links[i].src,
        grafo.links[i].dst,
        { weight: grafo.links[i].weight }
      );
    }
  } else {
    graph = new Graph({ type: "undirected" });
  ...
  for (let i = 0; i < grafo.vertex.length; i++) {
    ...
    for (let j = 0; j < grafo.vertex.length; j++) {
      if (shortestPath[grafo.vertex[i].id][j].peso === 1) {
        grafo.vertex[i].valore =
          grafo.vertex[i].valore + shortestPath[grafo.vertex[i].id][j].peso;
      }
    }
    //if (!pesato) grafo.vertex[i].valore = Math.round(grafo.vertex[i].valore / (grafo.vertex.length - 1)) / 1;
    if (pesato)
      grafo.vertex[i].valore = Math.floor(grafo.vertex[i].valore / 10) * 10;
...
const createGraph = (data, pesato) => {
  ...
  if (!pesato)
    for (let i = 0; i < grafoElaborato.vertex.length; i++) {
      ...
      if (
        grafoElaborato.vertex.length === i + 1 &&
        grafoElaborato.vertex[i].valore > start
      ) {
  ...
  if (pesato)
    for (let i = 0; i < grafoElaborato.vertex.length; i++) {
      ...
      if (
        grafoElaborato.vertex.length === i + 1 &&
        grafoElaborato.vertex[i].numeroAllenamenti > start
      ) {
        for (let j = i - 1; j >= ultimo_index; j--) {
          grafoElaborato.vertex[j].numero = contatore;
          grafoElaborato.vertex[j].coefficiente = j - ultimo_index;
  ...
  const maxLevel =
    grafoElaborato.vertex[grafoElaborato.vertex.length - 1].valore;
  const minLevelAllenamenti = grafoElaborato.vertex[0].numeroAllenamenti;
  const maxLevelAllenamenti =
    grafoElaborato.vertex[grafoElaborato.vertex.length - 1].numeroAllenamenti;
  if (!pesato) numeroLivelli = Math.max(10, maxLevel - minLevel + 1);
  if (pesato)
    numeroLivelli = Math.max(10, maxLevelAllenamenti - minLevelAllenamenti);
  ...
  if (pesato)
    var myColor = d3.scaleSequential(
      [minLevelAllenamenti - 10, maxLevelAllenamenti + 10],
      d3.interpolateRdYlGn
    );
  else
    var myColor = d3.scaleSequential(
      [minLevel - 50, maxLevel + 10],
      d3.interpolateRdYlGn
    );
  ...
  const chart = svg
    ...
    .attr(
      "transform",
      `translate(${(window.innerHeight * 1) / 2}, ${
  ...
  const svgHeight = window.innerHeight * 1;
  ...
  chart
    ...
    .attr("stroke-width", 4)
    .attr("cx", function (d) {
      var angoloGradi = Math.cos(
        (((360 / d.numero) * Math.PI) / 180) * d.coefficiente
      );
      if (!pesato) {
    ...
    .on("mouseover", function (event, d) {
      ...
      if (
        x < window.innerHeight / 2 - x * zoom - 25 &&
        y < window.innerHeight / 2 - y * zoom - 25
      ) {
        tooltip.attr(
          "transform",
          `translate(${window.innerHeight / 2 + x * zoom + 25}, ${
            window.innerHeight / 2 + y * zoom + 25
          })`
        );
      }
      if (
        ...
        tooltip.attr(
          "transform",
          `translate(${window.innerHeight / 2 + x * zoom + 25}, ${
            window.innerHeight / 2 + y * zoom - 50
          })`
        );
      }
      if (
        ...
        tooltip.attr(
          "transform",
          `translate(${window.innerHeight / 2 + x * zoom - 50}, ${
            window.innerHeight / 2 + y * zoom + 25
          })`
        );
      }
  ...
  d3.selectAll(".cc").on("click", function (event) {
    for (let i = 0; i < data.vertex.length; i++) {
      ...
      d3.select("#b" + data.vertex[i].id)
        .attr("stroke", "black")
        .attr("stroke-width", 4);
      if (this.id.replace("b", "") === data.vertex[i].id + "")
        data.vertex[i].selezionato = true;
    ...
    if (!pesato) {
      ...
      for (let i = 0; i < data.vertex.length; i++) {
        if (
          matriceDiAdiacenza[this.id.replace("b", "")][data.vertex[i].id]
            .peso === 1
        ) {
...
function App5() {
  ...
  useEffect(() => {
    const stringa =
      "https://mrkprojects.altervista.org/dataVis/adjMatrix.php?interval=W&index=" +
      settimane +
      "&year=" +
      anno;
    if (start) {
          ...
          d3.json(stringa).then((archi) => {
            ...
            for (let i = 0; i < vertici.length; i++) {
              for (let j = 0; j < archi.length; j++) {
                if (
                  vertici[i].id === archi[j].src ||
                  vertici[i].id === archi[j].dst
                ) {
                  vertici[i].valore = vertici[i].valore + 1;
                  vertici[i].numeroAllenamenti =
                    vertici[i].numeroAllenamenti + archi[j].weight;
                }
              }
              vertici[i].numeroAllenamenti =
                Math.floor(vertici[i].numeroAllenamenti / 5) * 5;
  ...
  return (
    <div className={styles.container}>
      ...
      <br />
      <InputLabel>
        Selezionare il metodo di visualizzazione tramite il{" "}
        <b>pulsante toggle</b>.
      </InputLabel>
      <InputLabel>
        Selezionare tramite <b>calendario</b> la settimana che si vuole
        monitorare
      </InputLabel>
      {pesato && (
        <InputLabel>
          In questa modalità <b>cliccando su un giocatore</b> si può monitorare
          quali giocatori nè il suddetto nè tutti i suoi compagni di allenamento
          non hanno mai giocato (è possibile valutare gruppi statici).
        </InputLabel>
      )}
      {!pesato && (
        <InputLabel>
          In questa modalità <b>cliccando su un giocatore</b> si può monitorare
          con quali altri giocatori ha effettuato allenamenti insieme (colore
          blu) e con quali non ha mai giocato (in trasparenza).
        </InputLabel>
      )}
      <InputLabel>
        <b>Cliccando su un giocatore</b> viene mantenuto il riferimento allo
        stesso tra le varie modalità.
      </InputLabel>
      <InputLabel>
        <b>Cliccando sullo spazio vuoto</b> si deseleziona il giocatore scelto.
      </InputLabel>
      <InputLabel>
        <b>Passando con il mouse</b> su una circonferenza è possibile vedere
        facilmente quanti giocatori ci sono.
      </InputLabel>
      <InputLabel>
        Tramite <b>rotella del mouse</b> è possibile effettuare lo zoom in e out
        sul grafico.
      </InputLabel>
      <br />
      <br />
